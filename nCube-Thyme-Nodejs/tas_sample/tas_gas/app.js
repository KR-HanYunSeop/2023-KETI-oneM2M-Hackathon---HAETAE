...
function control_motor(comm_num) {
	if ( comm_num == '1') {
	console.log("test_detected");
    var parent_process = cp.spawn("python3", ["./motor_open.py"]);	
    ...
    if ( comm_num == '0') {
        var parent_process = cp.spawn("python3", ["./motor_close.py"]);	
        parent_process.on('close', function (code) {
            console.log('Child process is exiting with exit code: ' + code);
	...
	console.log("test_init");
        var parent_process = cp.spawn("python3", ["./camera_to_server.py"]);
       // var parent_process = cp.spawn("python3", ["./motor_init.py"]);	
        parent_process.on('close', function (code) {
            console.log('Child process is exiting with exit code: ' + code);
...
//function control_camera(comm_num) {
//	if ( comm_num == '1') {
//	console.log('TEST!!');
//}
